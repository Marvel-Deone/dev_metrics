...
// type PRActivity = {
//     weekly: PRWeekActivity[];
// };

// // Mock data (can remain for placeholders)
// const commitTrendsData = [
//     { day: "Mon", commits: 12, additions: 450, deletions: 120 },
//     { day: "Tue", commits: 19, additions: 680, deletions: 230 },
//     { day: "Wed", commits: 8, additions: 220, deletions: 80 },
...
//         user,
//         repos = [],
//         commitTrends = commitTrendsData,
//         languages,
//         pullRequests,
//         contributionCalendar = heatmapData,
//         loading,
//         error
//     } = useGitHubData() || {}

//     const { prActivity }: { prActivity?: PRActivity } = useGitHubData() || {};

//     const username = session?.user?.login || "";
//     console.log('languages:', languages, 'PRActivity:', prActivity);
...
//     const [isRefreshing, setIsRefreshing] = useState(false)
//     const [mobileMenuOpen, setMobileMenuOpen] = useState(false)
//     const [prActivities, setPrActivities] = useState<PRWeekActivity[] | undefined>();
...
//         setIsRefreshing(false)
//     }
//     const weeklyActivities = prActivity?.weekly
//     // setPrActivities(prActivity.weekly);
...
// Mock Data
const commitTrendsData = [
    { day: "Mon", commits: 12 }, { day: "Tue", commits: 19 }, { day: "Wed", commits: 8 },
    { day: "Thu", commits: 24 }, { day: "Fri", commits: 15 }, { day: "Sat", commits: 6 },
    { day: "Sun", commits: 3 }
]
...
function DashboardContent() {
    const { theme, setTheme } = useTheme()
    const { data: session } = useSession()
    const { user, repos = [], activeRepos = [], commitTrends = commitTrendsData, languages, pullRequests, contributionCalendar = heatmapData, loading, error, prActivity } = useGitHubData() || {}
    const username = session?.user?.login || ""
    const metrics = useGithubMetrics(username || "Marvel-Deone")
    const languageData = toDashboardLanguages(languages ?? [])
    ...
    console.log('ActiveRepossss:', activeRepos);
    ...
    return (
        <div className="min-h-screen bg-background">
            ...
            <main className="p-4 md:p-6 max-w-7xl mx-auto">
                ...
                {/* ...keep the rest of your JSX as is, replacing prActivity safely with prActivity?.weekly ?? [] */}
                <div className="grid lg:grid-cols-3 gap-6 mb-6">
                    {/* Commit Trends */}
                    <Card className="lg:col-span-2 animate-slide-up stagger-1 border-border/50">
                        <CardHeader className="flex flex-row items-center justify-between pb-2">
